...
write_boot0           =      [0x55,0xAA,0x00,0xB3,0x01,0x00,0x00,0x02,0xB0,0x00]
write_boot1           =      [0x55,0xAA,0x00,0xA5,0x01,0x01,0x00,0x03,0xA0,0x00]
write_app             =      [0x55,0xAA,0x00,0x97,0x01,0x02,0x00,0x04,0x90,0x00]
data_head             =      [0x55,0xAA,0x00,0x01,0x01,0x00,0x00,0x00,0x00,0x00]
data_head_update_done =      [0x55,0xAA,0x00,0xD2,0x10,0x10,0x00,0x02,0xB0,0x00]
app_addr    =0x00049000
...
if __name__ == '__main__':
    ...
    #print(args.port)
    try:
        com = serial.Serial(args.port,115200,timeout=0.5)
        com.setDTR(True)
        com.setRTS(True)
    ...
    #binsize = os.path.getsize(args.url.replace('\\','/'))
    hello = 0
    ...
    if myhid.alive:
        myhid.setcallback()
        send_list = report_id + write_boot0 + flash_data
